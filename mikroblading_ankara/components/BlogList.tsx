...
export function BlogList({ posts }: { posts: PostList[] }) {
  if (!posts.length) {
    return (
      <p className="mt-8 rounded-[18px] border border-line bg-white px-6 py-10 text-center text-muted2">
  ...
  return (
    <div className="mt-8 grid gap-6 md:grid-cols-3">
      {posts.map((p) => (
        <article key={p.id} className="group overflow-hidden rounded-[20px] border border-line bg-white">
          {p.cover_url && (
            <Link href={`/blog/${p.slug}`}>
              <ImageSlot src={p.cover_url} alt={p.title_tr} ratio="aspect-[16/9]" />
            </Link>
          )}
          <div className="p-5">
            {p.published_at && (
