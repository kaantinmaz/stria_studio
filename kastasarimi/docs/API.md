# Kaş Tasarımı Ankara — Microsite API

Public, read-only content API served by the shared Stria Studio Laravel backend.
All content is scoped by the `site` slug. Machine-readable spec: [`openapi.yaml`](../public/openapi.yaml) · rendered: [`/api-docs`](https://kastasarimiankara.com/api-docs).

**Base URL:** `{API_URL}/api/microsites/{site}` — e.g. `http://127.0.0.1:8002/api/microsites/kas-tasarimi-ankara`
...
### GET /posts
```json
{ "data": [ { "id": 1, "slug": "kaş tasarımı-nedir", "title_tr": "…", "excerpt_tr": "…",
...
### GET /posts/{slug}
```json
{ "data": { "slug": "kaş tasarımı-nedir", "title_tr": "…", "body_tr": "<p>…</p>",
  "meta_title_tr": "…", "meta_desc_tr": "…" } }
```

### GET /faqs
```json
{ "data": [ { "q_tr": "Kaş Tasarımı nedir?", "a_tr": "…" } ] }
```
